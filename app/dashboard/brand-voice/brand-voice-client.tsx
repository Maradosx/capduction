...
export function BrandVoiceClient({ voices, isDemoMode }: { voices: BrandVoice[]; isDemoMode: boolean }) {
  const t = useT();
    ...
    <div className="max-w-[1200px] mx-auto flex flex-col gap-6">
      ...
      {voices.length === 0 ? (
        <div className="glass rounded-[18px] p-12 text-center">
          ...
          <p className="text-ink-3 text-[14px] mb-4 lang-th:font-thai">{t('bv.empty')}</p>
          <Link
            href="/dashboard/brand-voice/new"
            data-cursor="create"
              ...
              <div className="flex items-start gap-3 mb-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[11px] text-ink-3 font-mono mt-0.5">
                    {(v.sample_posts?.length ?? 0)} sample{v.sample_posts?.length !== 1 ? 's' : ''} · Updated {new Date(v.updated_at).toLocaleDateString(undefined, { day: 'numeric', month: 'short' })}
                  </div>
                </div>
              </div>
