...
export function ProjectsClient({ projects, isDemoMode }: { projects: Project[]; isDemoMode: boolean }) {
  const t = useT();
    ...
    <div className="max-w-[1200px] mx-auto flex flex-col gap-6">
      <div className="flex justify-between items-end flex-wrap gap-4">
        <div>
          <h1 className="font-display font-bold text-[clamp(28px,3.5vw,42px)] tracking-[-0.025em] text-ink lang-th:font-thai">
            <span className="text-iridescent">{t('pj.title')}</span>
          </h1>
          <p className="text-ink-3 text-[15px] mt-1 lang-th:font-thai">
            {t('pj.sub')}
            {isDemoMode && ` · ${t('pj.demo')}`}
          </p>
        </div>
        <Link
      ...
      {projects.length === 0 ? (
        <div className="glass rounded-[18px] p-12 text-center">
          ...
          <p className="text-ink-3 text-[14px] mb-4 lang-th:font-thai">{t('pj.empty')}</p>
          <Link
            href="/dashboard/projects/new"
            data-cursor="create"
              ...
              <div className="flex items-start gap-3 mb-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[11px] text-ink-3 font-mono mt-0.5">
                    Updated · {new Date(p.updated_at).toLocaleDateString(undefined, { day: 'numeric', month: 'short' })}
                  </div>
                </div>
              </div>
              {p.description && (
